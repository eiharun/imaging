            ...
            for(size_t c{}; c<result.channels; ++c){
                ...
                for(size_t kh{}; kh<m_k_height; ++kh){
                    for(size_t kw{}; kw<m_k_width; ++kw){
                        ...
                        signed long int w_idx = std::clamp((signed long int)w+kernel_w_offset, (signed long int)0, (signed long int)img->width-1);
                        signed long int h_idx = std::clamp((signed long int)h+kernel_h_offset, (signed long int)0, (signed long int)img->height-1);
                        size_t input_idx = (w_idx + result.width*h_idx);
                        sum += img->data[input_idx*result.channels + c] * m_kernel[kw + m_k_width*kh];
                    }
                }
...
void GaussianBlur::build_kernel(){
    ...
    for(signed int y{y_begin}; y<y_end+1; ++y){
        for(signed int x{x_begin}; x<x_end+1; ++x){
            m_kernel[(x+m_k_center_x) + m_k_width*(y+m_k_center_y)] = std::exp(-1*((x*x + y*y)/(2.0*m_sigma*m_sigma)));
            // m_kernel[(x+m_k_center_x) + m_k_width*(y+m_k_center_y)] = (1.0/(2*M_PI*m_sigma*m_sigma)) * std::exp(-1*((x*x + y*y)/(2.0*m_sigma*m_sigma)));
        }
    }
    double sum = 0.0;
    for (float v : m_kernel) sum += v;
    for (float& v : m_kernel) v /= sum;
    
    double s = 0.0;
    for (float v : m_kernel) s += v;
    std::cout << "kernel sum = " << s << "\n";
